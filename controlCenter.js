...
export async function main(ns) {
  ...
  // --- NEW: Kill bootstrap.js if found running anywhere ---
  async function killBootstrapIfRunning() {
    try {
      const running = findProcessByName("bootstrap.js");
      if (running) {
        ns.tprint(`🛑 controlCenter: bootstrap.js detected running on ${running.host} (PID ${running.pid}) — terminating.`);
        ns.kill(running.pid);
      }
    } catch (e) {
      ns.print(`⚠️ Error checking/killing bootstrap.js: ${String(e)}`);
    }
  }
  ...
  while (true) {
    try {
      // Always enforce bootstrap kill rule
      await killBootstrapIfRunning();
